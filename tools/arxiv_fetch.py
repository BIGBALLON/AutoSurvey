...
_USER_AGENT = "autosurvey/1.0 (+https://github.com/; AutoSurvey skill pack)"
_MAX_BACKOFF = 30  # cap any single sleep so a huge Retry-After can't stall a run
...
def _retry_after_seconds(exc: urllib.error.HTTPError) -> float | None:
    """Return the server-advertised Retry-After delay in seconds, if any.

    Honours the integer-seconds form (``Retry-After: 30``). The HTTP-date
    form is rare for these APIs and intentionally ignored in favour of our
    own backoff.
    """
...
def _fetch_atom(url: str, retries: int = 6) -> ET.Element:
    """Fetch an arXiv Atom feed and return the parsed XML root.

    arXiv's export API rate-limits aggressively (≈1 request / 3 s) and is
    often reached through a shared egress IP, so a sustained 429 is common.
    Retry on 429 / 5xx and transient network errors, honouring the server's
    ``Retry-After`` header when present and otherwise backing off
    exponentially (capped) rather than failing the whole search on the
    first throttle.
    """
    ...
    for attempt in range(retries):
        try:
            ...
            if exc.code in (429, 500, 502, 503, 504) and attempt < retries - 1:
                ...
                delay = advised if advised is not None else 3 * (attempt + 1)
            ...
            if attempt < retries - 1:
                time.sleep(min(3 * (attempt + 1), _MAX_BACKOFF))
                continue
            raise
    if last_exc:
        raise last_exc
    raise RuntimeError("arXiv fetch failed without an exception")
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.command == "search":
        try:
            ...
            # Degrade gracefully: a sustained arXiv 429 / network failure must
            # not abort the whole search step with a traceback. Emit a valid
            # empty result on stdout so a JSONL pipeline keeps going on the
            # other sources, and surface the cause on stderr.
            code = getattr(exc, "code", None)
            hint = " (rate-limited — try again later or lean on OpenAlex/S2)" if code == 429 else ""
            print(f"[arxiv_fetch] search failed: {exc}{hint}", file=sys.stderr)
            print("[]")
            return 0
