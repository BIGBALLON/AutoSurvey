...
class OpenAlexClient:
    ...
    def __init__(self, api_key: Optional[str] = None, email: Optional[str] = None):
        ...
        # Polite pool: a contact email routes requests to OpenAlex's faster,
        # more reliable rate bucket. Default to a contact env var (or a
        # generic mailbox) so bursts of survey-search queries don't get
        # throttled on the anonymous common pool.
        self.email = (
            email
            or os.environ.get("OPENALEX_EMAIL")
            or os.environ.get("AUTOSURVEY_VERIFY_EMAIL")
            or "autosurvey@users.noreply.github.com"
        )
        self.session = requests.Session()

        # Set user agent with email for polite pool
        self.session.headers.update({
            "User-Agent": f"AutoSurvey/1.0 (mailto:{self.email})"
        })
    ...
    def search_works(
        ...
        try:
            ...
            for work in data.get("results", []):
                # A single malformed record must not discard the whole
                # query's results — parse defensively and skip on failure.
                try:
                    results.append(self._parse_work(work))
                except Exception as exc:  # noqa: BLE001
                    print(
                        f"Warning: skipped one OpenAlex record ({exc})",
                        file=sys.stderr,
                    )
    ...
    def _parse_work(self, work: Dict) -> Dict:
        ...
        # OpenAlex returns explicit ``null`` (not just missing keys) for many
        # nested objects — primary_location, source, open_access, author —
        # so every ``.get(..., {})`` must be hardened with ``or {}`` / ``or []``.
        # Extract authors
        authors = []
        ...
        keywords = [k.get("display_name") for k in (work.get("keywords") or [])[:5]]
        ...
        return {
            ...
            # Outgoing citation edges (short OpenAlex ids). Populated on
            # single-work GETs; used by snowball_citations.py for recall
            # expansion. Empty list when the field is absent.
            "referenced_works": [
                (rw or "").split("/")[-1]
                for rw in (work.get("referenced_works") or [])
                if rw
            ],
            "url": work.get("id")
        }
    ...
    def fetch_citing(
        ...
        """Return works that CITE ``work_id`` (incoming citation edges).

        Used for citation-graph snowballing: the papers that cite a seed are
        a high-precision pool of on-topic, often more-recent work.
        """
        wid = (work_id or "").split("/")[-1]
        if not wid:
            return []
        params = {
            "filter": f"cites:{wid}",
        ...
        for w in resp.json().get("results", []):
            try:
                ...
                print(f"Warning: skipped one citing record ({exc})", file=sys.stderr)
        return out[:max_results]

    def get_works_by_ids(self, ids: List[str]) -> List[Dict]:
        """Batch-fetch parsed works by OpenAlex id (chunks of 50)."""
        short = [(i or "").split("/")[-1] for i in ids if i]
        out: List[Dict] = []
        for k in range(0, len(short), 50):
            chunk = short[k:k + 50]
            if not chunk:
                continue
            params = {
            ...
            try:
                ...
                for w in resp.json().get("results", []):
                    try:
                        ...
                        print(
                            f"Warning: skipped one batch record ({exc})",
                            file=sys.stderr,
                        )
            except Exception as exc:  # noqa: BLE001
                print(f"Warning: batch id fetch failed ({exc})", file=sys.stderr)
        return out
