...
def _request_json(url: str, *, retries: int = 4, timeout: int = _DEFAULT_TIMEOUT) -> dict[str, Any]:
    # S2's unauthenticated tier is shared and aggressively throttled, so a
    # sustained 429 is common from a busy egress IP. Retry several times,
    ...
    for attempt in range(retries + 1):
        try:
            ...
            # Best-effort: pull the response body for a more informative
            # error message. If the body is unreadable (closed stream,
            # decode failure, …) we fall back to an empty string and
            # keep the HTTP status code as the only diagnostic.
            try:
            ...
            if exc.code in (429, 500, 502, 503, 504) and attempt < retries:
                ...
                delay = advised if advised is not None else 1.5 * (attempt + 1)
            ...
            if attempt < retries:
                time.sleep(min(1.5 * (attempt + 1), _MAX_BACKOFF))
                last_err = exc
                continue
