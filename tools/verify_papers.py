...
Used by `/survey-search` (Step 6, mandatory).
...
Email for CrossRef User-Agent: reads `AUTOSURVEY_VERIFY_EMAIL` env, falls back to
`autosurvey-research@anonymous.local` (placeholder, not a real address). Set the env
to reduce CrossRef rate-limit risk:

  export AUTOSURVEY_VERIFY_EMAIL="you@institution.edu"
"""
...
def resolve_cache_path(scope: str, cache_dir: str | None) -> Path | None:
    ...
    if scope == "user":
        return Path.home() / ".autosurvey-cache" / "verify_papers.json"
    if scope == "project":
        return Path(".autosurvey/cache/verify_papers.json")
    return None
...
def save_cache(path: Path, cache: dict[str, dict[str, Any]]) -> None:
    if not path:
        return
    parent = path.parent
    # Announce + self-protect the project-scope cache directory the
    # first time it's created. Without this, users discover cwd-side
    # `.autosurvey/cache/` only via `git status` after the fact.
    cache_dir_was_new = not parent.exists()
    parent.mkdir(parents=True, exist_ok=True)
    if cache_dir_was_new:
        # Drop a local .gitignore so the cache is always self-shielded
        # even when the surrounding repo's .gitignore has not been
        # updated. Idempotent: only written if missing.
        gi = parent / ".gitignore"
        if not gi.exists():
            gi.write_text("# auto-created by tools/verify_papers.py\n*\n")
        try:
            relative = parent.resolve().relative_to(Path.cwd().resolve())
            shown = str(relative)
        except ValueError:
            shown = str(parent)
        print(f"[verify_papers] cache dir created → {shown}/ "
              "(self-shielded with .gitignore)",
              file=sys.stderr)
...
def verify_doi(doi: str, user_email: str) -> str:
    ...
    headers = {"User-Agent": f"AutoSurvey-verify-papers/1.0 (mailto:{user_email})"}
...
def main() -> int:
    ...
    user_email = os.environ.get("AUTOSURVEY_VERIFY_EMAIL", "autosurvey-research@anonymous.local").strip()
